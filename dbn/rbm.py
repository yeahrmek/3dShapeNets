...
from rbm_layer import _calc_optree
...
class RBM(Model):
    ...
    def _epoch_fit(self, dataset, callbacks):
        ...
        for mb_idx, (x, t) in enumerate(dataset):
            ...
            self.optimizer.optimize([layer], epoch)
    ...
    def wake_sleep(self, dataset, optimizer, num_epochs, callbacks):
        ...
        if not self.layers.layers_to_optimize[-1].chain is None:
            self.layers.layers_to_optimize[-1].chain = None
    ...
    def _epoch_wake_sleep(self, dataset, callbacks):
        ...
        self.ws_update_params = {'persistant': self.optimizer['persistant'],
        ...
        for mb_idx, (x, t) in enumerate(dataset):
            ...
            for i, l in enumerate(layers[:-1]):
                wake_states += [l.fprop(wake_states[i], labels=t, weights=l.UW)]
                wake_states[-1] = _calc_optree(wake_states[-1] > self.be.array(l.be.rng.uniform(size=wake_states[-1].shape)), l.be)

            # updates for top rbm. It slightly differs from
            update_top_W, update_top_b_hidden, update_top_b_visible, v_neg_top, v_neg_sample_top, h_neg_top = \
                layers[-1].update_for_wake_sleep(wake_states[-1], t, **self.ws_update_params)
            ...
            sleep_activations[-1] = _calc_optree(h_neg_top, layers[-1].be)
            sleep_activations[-2] = v_neg_top[layers[-1].n_classes:]
            sleep_states = [[]] * (n_layers + 1)
            sleep_states[-1] = layers[-1].chain
            sleep_states[-2] = v_neg_sample_top[layers[-1].n_classes:]

            # generating top - down
            for i, l in reversed(list(enumerate(layers[:-1]))):
                sleep_activations[i] = l.bprop(sleep_states[i + 1], weights=l.DW)
                if i == 0:
                    sleep_states[0] = sleep_activations[0]
                else:
                    sleep_states[i] = _calc_optree(sleep_activations[i] > l.be.array(
                                                   l.be.rng.uniform(size=sleep_activations[i].shape)),
                                                   l.be)
            ...
            p_rec = []
            for i, l in enumerate(layers[:-1]):
                p_gen += [l.bprop(wake_states[i + 1], weights=l.DW)]
                p_rec += [l.fprop(sleep_states[i])]


            ## update parameters
            # top rbm
            layers[-1].dW[:] = momentum * layers[-1].dW + lr  * update_top_W
            layers[-1].db_hidden[:] = momentum * layers[-1].db_hidden + lr  * update_top_b_hidden
            layers[-1].db_visible[:] = momentum * layers[-1].db_visible + lr * update_top_b_visible
            ...
            #generative weights
            for i, l in enumerate(layers[:-1]):
                l.dDW[:] = momentum * l.dDW + lr * l.n_hidden * \
                           l._grad_W(wake_states[i] - p_gen[i], wake_states[i + 1]) / l.be.bsz
                l.db_visible[:] = momentum * l.db_visible + lr * l._grad_b_visible(wake_states[i], p_gen[i])
                l.DW[:] = l.DW + l.dDW
            ...
            #TODO: check this
            for i, l in enumerate(layers[:-1]):
                l.dUW[:] = momentum * l.dUW + lr * l.n_hidden * \
                           l._grad_W(sleep_activations[i], sleep_states[i + 1] - p_rec[i]) / l.be.bsz
                l.db_hidden[:] = momentum * l.db_hidden + lr * l._grad_b_hidden(sleep_states[i + 1], p_rec[i])
                l.UW[:] = l.UW + l.dUW
